...
The React frontend uses `VITE_API_BASE` environment variable (fallback to `http://127.0.0.1:8000`).
...
For production with a deployed backend (Render, Fly, etc.), use:
```env
VITE_API_BASE=https://your-fastapi-service.onrender.com
```

CORS is enabled in `main.py` for:
- `http://localhost:5173`
- `http://127.0.0.1:5173`
